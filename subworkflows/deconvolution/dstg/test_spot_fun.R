...
test_spot_fun <- function(se_obj,
  ...
  suppressMessages(require(scuttle)) # For the downsampling (replaced DropletUtils with this)
  ...
  ds_spots <- lapply(seq_len(n), function(i) {
    ...
    if (sum(syn_spot) > 25000) {
      syn_spot_sparse <- scuttle::downsampleMatrix(Matrix::Matrix(syn_spot, sparse = T),
